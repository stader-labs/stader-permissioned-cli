...
func recoverWallet(c *cli.Context) error {
	...
	if addressString != "" {
		...
		// Recover wallet
		response, err := staderOwner.SearchAndRecoverWallet(mnemonic, address, true)
		if err != nil {
			return err
		}
		...
		// Recover wallet
		response, err := staderOwner.RecoverWallet(mnemonic, true, derivationPath, walletIndex)
		if err != nil {
			return err
		}
