...
func registerNode(c *cli.Context) error {
	...
	if canRegister.OperatorNotWhitelisted {
		fmt.Println("The node is not whitelisted.")
		return nil
	}
	...
	// Register node
	response, err := staderClient.RegisterNode(operatorName, common.HexToAddress(operatorRewardAddressString))
	if err != nil {
		return err
	}
