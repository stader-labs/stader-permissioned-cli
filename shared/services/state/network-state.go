...
func CreateNetworkStateCache(c *cli.Context, cfg *config.StaderNodeConfig, ec stader.ExecutionClient, bc beacon.Client, log *log.ColorLogger, slotNumber uint64, beaconConfig beacon.Eth2Config, nodeAddress common.Address) (*NetworkStateCache, error) {
	...
	prn, err := stader.NewPermissionedNodeRegistry(ec, prnAddress)
	if err != nil {
		return nil, err
	}
