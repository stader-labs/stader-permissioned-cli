...
func (c *StandardHttpClient) GetForkInfo() (beacon.ForkInfo, error) {
	forkResponse := beacon.ForkInfo{}

	res, err := c.getFork("head")
	if err != nil {
		return beacon.ForkInfo{}, err
	}

	forkResponse.PreviousVersion = res.Data.PreviousVersion
	forkResponse.CurrentVersion = res.Data.CurrentVersion
	forkResponse.Epoch = uint64(res.Data.Epoch)

	return forkResponse, nil
}

func (c *StandardHttpClient) GetGenesisInfo() (beacon.GenesisInfo, error) {
	genesisResponse := beacon.GenesisInfo{}

	res, err := c.getGenesis()
	if err != nil {
		return beacon.GenesisInfo{}, err
	}

	genesisResponse.GenesisTime = uint64(res.Data.GenesisTime)
	genesisResponse.GenesisForkVersion = res.Data.GenesisForkVersion
	genesisResponse.GenesisValidatorsRoot = res.Data.GenesisValidatorsRoot
